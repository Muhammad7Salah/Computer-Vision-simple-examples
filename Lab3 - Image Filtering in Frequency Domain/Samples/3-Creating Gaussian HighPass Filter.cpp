...
{    
	Mat img;
		...
		Shift(filter1);
		Shift(filter2);
     Mat kernel1 = GenerateILPF(20,filter1.cols,filter1.rows);
//	Mat kernel = createGaussianHighPassFilter(Size(300, 300), 20.0);
	  ApplyFilter(filter1,kernel1,filter1); //////////
		ApplyFilter(filter2,kernel1,filter2);//////////
